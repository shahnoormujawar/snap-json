...
interface Frame {
  closer: '}' | ']';
  /** Index just past the opening bracket or the last comma at this depth — a safe truncation point. */
  cut: number;
...
export function repairJson(raw: string): string {
  ...
  let stringStart = -1;
  let escape: 'none' | 'char' | 'unicode' = 'none';
  let escapeStart = -1;
  let unicodeDigits = 0;
  ...
  for (let i = 0; i < text.length; i++) {
    ...
    if (inString) {
      ...
      if (escape === 'unicode') {
        unicodeDigits++;
        if (unicodeDigits === 4) escape = 'none';
        continue;
      }
      if (c === '\\') {
        escape = 'char';
  ...
  if (inString) {
    ...
    while (/[.eE+\-]$/.test(out)) out = out.slice(0, -1);
    out = out.trimEnd();
  ...
  for (let i = stack.length - 1; i >= 0; i--) out += stack[i].closer;
  return out;
}

export class StreamingJsonParser {
  private buffer = '';
  private lastGood: Record<string, any> = {};
  private lastGoodKey = '';
    ...
    if (repaired.length > 0) {
      try {
        ...
        if (value !== null && typeof value === 'object') {
          this.lastGood = value as Record<string, any>;
...
export function parseStreamChunk(rawTextChunk: string): Record<string, any> {
