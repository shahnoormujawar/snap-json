...
export class PayloadHealer {
  ...
  private readonly snippetRadius: number;
   ...
   */
  async healPayload(brokenJson: string, errorMsg: string, fallbackClient: FallbackClient): Promise<any> {
    ...
    throw new HealingError(
      `healing failed after ${this.maxAttempts} snippet attempt(s)`,
      lastError,
    );
  }
...
export async function healPayload(
  brokenJson: string,
  errorMsg: string,
  fallbackClient: FallbackClient,
): Promise<any> {
...
export function validatePayload(
  ...
  if (value === null) {
    if (schema.nullable || schema.type === 'null') return issues;
 ...
 */
export function pruneUnknownKeys(value: unknown, schema: JsonSchema): unknown {
  if (value === null || typeof value !== 'object') return value;
  if (schema.type === 'array' && Array.isArray(value)) {
    const items = schema.items;
    if (items) {
      for (let i = 0; i < value.length; i++) value[i] = pruneUnknownKeys(value[i], items);
    }
    return value;
  }
...
function cleanModelReply(reply: string): string {
  let out = reply.trim();
...
function snippetPrompt(snippet: string, errorMsg: string): string {
  return [
...
function wholeDocumentPrompt(document: string, errorMsg: string): string {
  return [
...
function extractErrorPosition(errorMsg: string, text: string): number {
  const positionMatch = /position (\d+)/i.exec(errorMsg);
  if (positionMatch) return Math.min(Number(positionMatch[1]), text.length);
...
function positionFromLineColumn(text: string, line: number, column: number): number {
  let index = 0;
...
function getAtPath(root: unknown, segments: ReadonlyArray<string | number>): unknown {
...
function setAtPath(root: unknown, segments: ReadonlyArray<string | number>, next: unknown): unknown {
  ...
  if (current !== null && typeof current === 'object') {
    (current as Record<string, unknown>)[String(segments[segments.length - 1])] = next;
  }
  return root;
}
