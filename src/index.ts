...
const DEFAULT_HEALING_MODELS: Record<Provider, string> = {
  openai: 'gpt-4o-mini',
...
export class SnapJson {
  private readonly translator = new SchemaTranslator();
  private readonly fetchImpl: (input: string, init?: RequestInit) => Promise<Response>;

  constructor(private readonly config: SnapJsonConfig) {
    const fetchFn = config.fetch ?? (typeof fetch !== 'undefined' ? fetch : undefined);
    if (!fetchFn) {
      throw new SnapJsonError(
        'No fetch implementation available: pass one via config.fetch or run on a platform with global fetch.',
      );
    }
  ...
  async extract<T>(prompt: string, options: ExtractionOptions): Promise<T> {
    ...
    return this.finalize<T>(rawText, translated.schema, provider, options);
  }
  ...
  async streamObject<T>(
    ...
    if (!response.ok) {
      ...
      throw new SnapJsonError(
        `${provider} stream request failed (HTTP ${response.status}): ${detail}`,
        provider,
        response.status,
      );
    }
    ...
    for await (const event of sseEvents(response.body)) {
      ...
      if (emitEveryDelta || parser.snapshotKey !== lastSnapshot) {
        ...
        onChunk(partial as Partial<T>);
      }
    }
    return this.finalize<T>(parser.raw, translated.schema, provider, options);
  }
  ...
  private async finalize<T>(
    ...
    try {
      ...
      if (!repaired) {
        if (!healing.enabled) {
          throw new SnapJsonError(
            `output is not valid JSON: ${errorText(parseError)}`,
            provider,
            undefined,
            undefined,
            rawText,
        ...
        try {
          value = await healer.healPayload(
            cleaned,
            errorText(parseError),
            this.makeFallbackClient(provider, healing, options.signal),
          );
        } catch (healError) {
          throw new SnapJsonError(
            `output is not valid JSON and healing failed: ${errorText(healError)}`,
            provider,
            undefined,
            undefined,
            rawText,
    ...
    if (issues.length > 0 && healing.enabled) {
      ...
      value = await healer.healValidation(
        value,
        schema,
        this.makeFallbackClient(provider, healing, options.signal),
      );
    ...
    if (issues.length > 0) {
      throw new SnapJsonError(
        `output failed schema validation: ${issues.map((i) => `${i.path} — ${i.message}`).join('; ')}`,
        provider,
        undefined,
        issues,
        rawText,
  ...
  private makeFallbackClient(
    ...
    if (!this.config.apiKeys[provider]) provider = primary;
    const model = healing.model ?? DEFAULT_HEALING_MODELS[provider];
    return async (prompt: string): Promise<string> => {
      const request = this.buildRequest(
        provider,
        prompt,
        { model, temperature: 0, maxOutputTokens: HEALING_MAX_OUTPUT_TOKENS },
        {},
        false,
      );
  ...
  private buildRequest(
    ...
    switch (provider) {
      ...
      case 'gemini': {
        const fragmentGeneration = (fragment.generationConfig ?? {}) as Record<string, unknown>;
  ...
  private async send(provider: Provider, request: WireRequest, signal?: AbortSignal): Promise<any> {
    const response = await this.fetchImpl(request.url, {
      method: 'POST',
      headers: request.headers,
      body: JSON.stringify(request.body),
    ...
    if (!response.ok) {
      const detail = (await response.text().catch(() => '')).slice(0, 600);
      throw new SnapJsonError(
        `${provider} request failed (HTTP ${response.status}): ${detail}`,
        provider,
        response.status,
      );
    }
  ...
  private extractText(provider: Provider, response: any): string {
    switch (provider) {
      case 'openai': {
      ...
      case 'anthropic': {
        ...
        if (toolUse && toolUse.input !== undefined) return JSON.stringify(toolUse.input);
        const text = blocks
        ...
        if (text) return text;
        throw new SnapJsonError(
          `anthropic response contained no tool_use or text content (stop_reason: ${response?.stop_reason ?? 'unknown'})`,
          provider,
        );
      }
      case 'gemini': {
        ...
        throw new SnapJsonError(
          `gemini response contained no text${blockReason ? ` (blocked: ${blockReason})` : ''}`,
          provider,
        );
      }
    }
  ...
  private extractDelta(provider: Provider, event: any): string {
    switch (provider) {
      case 'openai': {
  ...
  private resolveProvider(options: RequestOptions): Provider {
    const provider = options.provider ?? this.config.defaultProvider;
    if (!provider) {
      throw new SnapJsonError(
        'No provider specified: set options.provider or config.defaultProvider.',
      );
    }
    return provider;
  }

  private apiKey(provider: Provider): string {
    const key = this.config.apiKeys[provider];
    if (!key) throw new SnapJsonError(`Missing API key for provider "${provider}"`, provider);
    return key;
  }

  private resolveHealing(options: RequestOptions): ResolvedHealing {
 ...
 */
async function* sseEvents(stream: ReadableStream<Uint8Array>): AsyncGenerator<any, void, unknown> {
