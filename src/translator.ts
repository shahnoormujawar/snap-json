...
const PRIMITIVE_HINTS: ReadonlySet<string> = new Set(['string', 'number', 'integer', 'boolean']);

export class SchemaTranslator {
  translate(template: SchemaTemplate, provider: Provider, name = 'emit_json'): TranslatedSchema {
  ...
  normalize(template: SchemaTemplate): JsonSchema {
    if (this.isExplicitSchema(template)) return this.sanitize(template);
    return this.infer(template);
  }

  private isExplicitSchema(template: SchemaTemplate): template is JsonSchema {
    if (typeof template !== 'object' || template === null || Array.isArray(template)) return false;
    const type = (template as { type?: unknown }).type;
    if (typeof type !== 'string' || !TYPE_NAMES.has(type)) return false;
    if (type === 'object') return 'properties' in template;
    if (type === 'array') return 'items' in template;
    return true;
  }

  /** Deep-copy an explicit schema, keeping only supported fields and filling defaults. */
  private sanitize(schema: JsonSchema): JsonSchema {
  ...
  private infer(value: unknown): JsonSchema {
    if (typeof value === 'string') return this.hintToSchema(value);
  ...
  private hintToSchema(hint: string): JsonSchema {
    ...
    const schema: JsonSchema = { type: 'string' };
    if (trimmed.length > 0) schema.description = trimmed;
    return schema;
  }
   ...
   */
  private emitStandard(schema: JsonSchema, strict: boolean): Record<string, unknown> {
    ...
    if (schema.type === 'object') {
      ...
      for (const key of keys) properties[key] = this.emitStandard(source[key], strict);
      out.properties = properties;
  ...
  private emitGemini(schema: JsonSchema): Record<string, unknown> {
    const out: Record<string, unknown> = {
      type: schema.type === 'null' ? 'STRING' : schema.type.toUpperCase(),
    };
    if (schema.description) out.description = schema.description;
    if (schema.nullable || schema.type === 'null') out.nullable = true;
    if (schema.enum && schema.type === 'string') out.enum = schema.enum;
    if (schema.type === 'object') {
      const source = schema.properties ?? {};
      const keys = Object.keys(source);
      const properties: Record<string, unknown> = {};
      for (const key of keys) properties[key] = this.emitGemini(source[key]);
      out.properties = properties;
      if (keys.length > 0) {
